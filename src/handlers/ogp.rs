...
use crate::db::{prices, share_cards, snapshots, ticker_share_cards};
...
fn delta_str(current: f64, prev: Option<f64>, unit: &str, integer: bool) -> String {
    let Some(p) = prev.filter(|&p| p > 0.0) else {
        return "—".to_string();
    };
    let d = current - p;
    let pct = (d / p) * 100.0;
...
fn chart_line_path(values: &[f64], x: f64, y: f64, w: f64, h: f64) -> Option<String> {
    ...
    for (i, &v) in values.iter().enumerate() {
        if i == 0 {
            path.push_str(&format!("M {:.1},{:.1}", to_x(i), to_y(v)));
        } else {
            path.push_str(&format!(" L {:.1},{:.1}", to_x(i), to_y(v)));
        }
    }
    Some(path)
}
...
fn portfolio_svg(input: PortfolioSvg<'_>) -> String {
    ...
    let pnl_pct = if total_cost_jpy > 0.0 {
        (unrealized_pnl_jpy / total_cost_jpy) * 100.0
    } else {
        0.0
    };
    let pnl_color = if unrealized_pnl_jpy >= 0.0 {
        "#34D399"
    } else {
        "#F87171"
    };
    ...
    let pnl_str = xml_escape(&format!(
        "{}¥{}  ({})",
        pnl_sign,
        format_with_commas(unrealized_pnl_jpy.abs()),
        signed_pct(pnl_pct)
    ));
    ...
    let symbols_str: String = sorted
        .iter()
        .take(6)
        .map(|h| xml_escape(&h.symbol))
        .collect::<Vec<_>>()
        .join("  ·  ");
...
fn ticker_svg(input: TickerSvg<'_>) -> String {
    ...
    let price_str = xml_escape(&format!(
        "{}{}",
        unit,
        format_with_commas(issue_price_native)
    ));
    ...
    let day_str = xml_escape(&delta_str(
        issue_price_native,
        prev_day_price,
        unit,
        integer,
    ));
    let month_str = xml_escape(&delta_str(
        issue_price_native,
        prev_month_price,
        unit,
        integer,
    ...
    let pnl_lines = match (quantity, avg_cost_native, issue_pnl_jpy) {
        (Some(qty), Some(avg), Some(pnl)) if qty > 0.0 => {
            let pnl_pct = if avg > 0.0 {
                ((issue_price_native - avg) / avg) * 100.0
            } else {
                0.0
            };
...
pub async fn share_ogp(
    ...
    let holdings: Vec<FundHoldingSnapshot> =
        serde_json::from_str(&card.holdings_json).map_err(|error| {
            tracing::error!(%error, "share ogp: invalid holdings snapshot");
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    ...
    let chart_values: Vec<f64> = snapshots::list_snapshots(&state.db)
        .await
        .map_err(|error| {
            tracing::error!(%error, "share ogp: failed to load chart history");
            StatusCode::INTERNAL_SERVER_ERROR
        })?
        .into_iter()
    ...
    let prev_day_value =
        snapshots::get_snapshot_on_or_before(&state.db, issue_date - Duration::days(1))
            .await
            .map_err(|error| {
                tracing::error!(%error, "share ogp: failed to load previous-day snapshot");
                StatusCode::INTERNAL_SERVER_ERROR
            })?
            .map(|s| s.total_value_jpy);
    let prev_month_value =
        snapshots::get_snapshot_on_or_before(&state.db, issue_date - Duration::days(30))
            .await
            .map_err(|error| {
                tracing::error!(%error, "share ogp: failed to load previous-month snapshot");
                StatusCode::INTERNAL_SERVER_ERROR
            })?
            .map(|s| s.total_value_jpy);
    ...
    let png = tokio::task::spawn_blocking(move || render_png(&svg))
        .await
        .map_err(|e| {
            tracing::error!("share ogp: join: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?
        .map_err(|e| {
            tracing::error!("share ogp: render: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
...
pub async fn ticker_ogp(
    ...
    let chart_values: Vec<f64> = prices::list_history(&state.db, &card.symbol, issue_date)
        .await
        .map_err(|error| {
            tracing::error!(%error, "ticker ogp: failed to load chart history");
            StatusCode::INTERNAL_SERVER_ERROR
        })?
        .into_iter()
        .filter(|(d, _)| *d >= since)
    ...
    let prev_day_price =
        prices::get_price_on_or_before(&state.db, &card.symbol, issue_date - Duration::days(1))
            .await
            .map_err(|error| {
                tracing::error!(%error, "ticker ogp: failed to load previous-day price");
                StatusCode::INTERNAL_SERVER_ERROR
            })?;
    let prev_month_price =
        prices::get_price_on_or_before(&state.db, &card.symbol, issue_date - Duration::days(30))
            .await
            .map_err(|error| {
                tracing::error!(%error, "ticker ogp: failed to load previous-month price");
                StatusCode::INTERNAL_SERVER_ERROR
            })?;
    ...
    let png = tokio::task::spawn_blocking(move || render_png(&svg))
        .await
        .map_err(|e| {
            tracing::error!("ticker ogp: join: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?
        .map_err(|e| {
            tracing::error!("ticker ogp: render: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
